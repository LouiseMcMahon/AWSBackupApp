...
from source.file import File,path_normalise,path_leaf,scan_folder
from pprint import pprint

class TestFile(TestCase):
    def test_init(self):
        import os
        read_file_loc = path_normalise(os.path.join(os.getcwd(), "test_files", "init_config_1_test.json"))
    ...
    def test_scan_folder(self):
        ...
        expected_recursive_no_ignore = [
            path_normalise(os.path.join(os.getcwd(), 'test_files', 'init_config_1_test.json')),
            path_normalise(os.path.join(os.getcwd(), 'test_files', 'test.csv')),
            path_normalise(os.path.join(os.getcwd(), 'test_files', 'test.php')),
            path_normalise(os.path.join(os.getcwd(), 'test_files', 'dont_ignore', 'test.txt')),
            path_normalise(os.path.join(os.getcwd(), 'test_files', 'ignore', 'test.json'))
        ]
        ...
        self.assertItemsEqual(scan_folder(test_files_path),
                         expected_recursive_no_ignore,
                         "recursive with no ignored folders failed")

        self.assertItemsEqual(scan_folder(test_files_path,False),
                         expected_no_recursive,
                         "none recursive failed")

        self.assertItemsEqual(scan_folder(test_files_path,True,[os.path.join(test_files_path,'ignore')]),
                         expected_recursive_ignore,
                         "recursive with ignored folders failed")

    def test_path_normalise(self):
        import os
        self.assertEqual(path_normalise(os.getcwd()), os.path.normcase(os.path.normpath(os.getcwd())), "path not normalised")

    def test_path_leaf(self):
        import os
        read_file_loc = os.path.join(os.getcwd(), "test_files", "init_config_1_test.json")
        file_name = path_leaf(read_file_loc)
